...
from .models import SaleDetail, Sale, Client, Quote, QuoteDetail
...
class SaleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        if data['document_type'] == 'FAC' and not data.get('client_rut'):
            raise serializers.ValidationError(
                "Facturas requieren RUT de cliente")
        return data
    ...
    def create(self, validated_data):
        ...
        for detail_data in details_data:
            ...
            if product.iva_tax:
                iva_total += (unit_price * quantity) - \
                    (unit_price * quantity / 1.19)
...
class QuoteDetailSerializer(serializers.ModelSerializer):
    id = ObjectIdField(read_only=True)
    product = serializers.PrimaryKeyRelatedField(queryset=Product.objects.all(), pk_field=ObjectIdField())
...
class QuoteSerializer(serializers.ModelSerializer):
    id = ObjectIdField(read_only=True)
    client = serializers.PrimaryKeyRelatedField(queryset=Client.objects.all(), pk_field=ObjectIdField())
    ...
    def create(self, validated_data):
        ...
        total = sum(detail['unit_price'] * detail['quantity'] for detail in details_data)
