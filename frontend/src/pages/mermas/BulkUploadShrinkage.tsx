...
const BulkUploadShrinkage: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await uploadShrinkageExcel(formData);
      setSnackbarMessage("Mermas cargadas exitosamente.");
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await downloadShrinkageTemplate();
      const blob = new Blob([response.data], { type: response.headers["content-type"] });
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.setAttribute("download", "plantilla_mermas.xlsx");
      ...
      <Card variant="outlined" sx={{ maxWidth: "100%", mx: "auto", p: 3, borderRadius: "sm" }}>
        <Box textAlign="center" sx={{ mb: 3 }}>
          ...
          <Typography level="body-sm">
            Arrastra y suelta tu archivo Excel para registrar múltiples mermas
          </Typography>
        </Box>
...
export default BulkUploadShrinkage;