...
import { Client, Quote, QuoteDetail } from '../../../types/sales.types'
...
const QuoteEditPage = () => {
    ...
    const { data: quote, isLoading: isLoadingQuote, isError: isErrorQuote } = useQuery<Quote>({
        queryKey: ['quote', id],
        queryFn: async () => {
            const response = await AxiosInstance.get<Quote>(`/api/sales/quotes/${id}/`)
    ...
    const { data: quoteClient } = useQuery({
        queryKey: ['quote-client', quote?.client.id],
        queryFn: () => fetchClient(quote?.client.id as string),
        enabled: !!quote?.client.id,
    })
    ...
    const clientsToOptions = (clients?: Client[]): SelectOption[] => {
        ...
        const baseOptions = clients?.map((client) => ({
            ...
            if (quoteClient && !baseOptions.find(opt => opt.value === quoteClient.id)) {
            baseOptions.push({
                value: quoteClient.id,
                label: quoteClient.first_name + ' ' + quoteClient.last_name,
            })
        }
...
const selectedClient = clients?.results.find((client: Client) => client.id === watch('client_id')) ?? quoteClient
